...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate in TSS2_CAR or ret.enableGasInterceptorDEPRECATED:
      #tune.kpBP = [0., 4.,  5.,   6.,   20., 30] if sp_tss2_long_tune else [0., 5., 20.]
      #tune.kpV = [0., 0., 0.001, 0.003, .1, .15] if sp_tss2_long_tune else [1.3, 1.0, 0.7]
      tune.kiBP = [0.,   0.1,   1.,   2.,    3.,   5.,  8.,  12., 20., 27., 36., 50] if sp_tss2_long_tune else [0., 5., 12., 20., 27.]
      tune.kiV = [.348, .351,  .33,  .3085, .286, .227, .221, .20, .17, .10, .10, .01] if sp_tss2_long_tune else [.35, .23, .20, .17, .1]
      if candidate in TSS2_CAR:
        ret.vEgoStopping = 0.25 if sp_tss2_long_tune else 0.25
        ret.vEgoStarting = 0.25 if sp_tss2_long_tune else 0.25
        ret.stopAccel = -0.40 if sp_tss2_long_tune else 0
        ret.stoppingDecelRate = 0.35 if sp_tss2_long_tune else 0.3  # reach stopping target smoothly
    else:
      tune.kpBP = [0., 5., 35.]
      tune.kiBP = [0., 35.]
